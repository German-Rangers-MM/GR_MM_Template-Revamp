...
_Vehicles = getMissionConfigValue "AFBHelos";
...
while {sleep 60, true} do {
	...
	_endPos = _target getPos [_distance,selectRandom [180,0]];
	
	// Spawning the Vehicle
	_AFBVehicle = createVehicle [_Vehicle, _startPos, [], 0, "FLY"];
	_AFBVehicle setPos [(getpos _AFBVehicle # 0),(getpos _AFBVehicle # 1), 75];
	...
	_AFBVehicle flyInHeight 75;

	_Pilot setCaptive true;
	
	//Setting the Waypoint for the Supply Drop Delivery
	_Pilot MoveTo _playerPos;

	WaitUntil {sleep 2; (_playerPos distance2D _AFBVehicle < 200)};
